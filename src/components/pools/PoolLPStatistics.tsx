  ...
  return <Card className="mb-4">
      <CardContent className="pt-6 py-[7px]">
        ...
        <div className="grid grid-cols-5 gap-2 mb-1 border-b pb-1">
          ...
          {periods.map((period, index) => <div key={index} className="font-medium text-xs">{period}</div>)}
        </div>
        ...
        <div className="grid grid-cols-5 gap-2 py-1 border-b border-border/30">
          ...
          {tvlChange.map((item, index) => <div key={index} className="flex items-center">
              {item.value > 0 ? <ArrowUpRight className="h-3.5 w-3.5 text-success mr-1" /> : <ArrowDownRight className="h-3.5 w-3.5 text-destructive mr-1" />}
        ...
        <div className="grid grid-cols-5 gap-2 py-1">
          ...
          {newPositions.map((item, index) => <div key={index} className="text-xs font-medium">{item.opened}</div>)}
        </div>
        
        {/* Closed Positions Row */}
        <div className="grid grid-cols-5 gap-2 py-1">
          <div className="text-xs text-muted-foreground">Closed</div>
          {newPositions.map((item, index) => <div key={index} className="text-xs font-medium">{item.closed}</div>)}
        </div>
      </CardContent>
    </Card>;
};
